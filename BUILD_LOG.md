...
22/09/2026 (commit 240b855 "Fix invisible white "Suggested image" label text in dark mode")
Time spent: ~5 min (measured - time since previous commit)
Tokens: ~5K (rough estimate)
Shipped: User-reported bug right after the rectangle-placeholder change: the bold "Suggested image" label was rendering white on the box's light background. Cause: the page declares color-scheme: light dark at :root, and .image-placeholder-label had no explicit text color, so under a dark OS/browser color scheme it inherited the default light foreground - unreadable against the box's fixed light background (#f5f5f7), which doesn't itself adapt to color scheme. Set explicit dark colors on .image-placeholder and its label so the box's readability no longer depends on the viewer's color scheme.
